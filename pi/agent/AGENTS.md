...
The global `permission-gate` extension is the enforcement boundary for destructive shell commands. Never evade it by splitting, encoding, aliasing, scripting, or rephrasing an operation. Approval is valid only for the exact command shown and only once. Only the user may pause the gate with `/permissions off`; never invoke, request, or recommend disabling it to complete a task. If a headless child/subagent is blocked, return the command and rationale to the parent for an explicit user decision; do not retry through another mechanism. Load the `safe-operations` skill when destructive work is necessary.
